...
mkdir test_results
lang=ro
for model in model/*bigger*bleu*decoder.yml
do
...
cat ../any-any/test.snt  |  bash ../../../add_tags.sh "$lang"  | bash trans_any_params.sh -c model/$model -d 0  -b 8 -n 1.0 --mini-batch 8 > test.out
