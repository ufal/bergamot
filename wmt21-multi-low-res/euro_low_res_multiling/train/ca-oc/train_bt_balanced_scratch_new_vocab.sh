...
$marian/marian \
	--model model/model.ca-$tgt_lang.bt.balanced.from.scratch.new_vocab.npz  --type transformer \
    --train-sets ../../data/lang_pairs/ca-$tgt_lang/all_text+bt_balanced.ca-$tgt_lang.cln.ca.sp ../../data/lang_pairs/ca-$tgt_lang/all_text+bt_balanced.ca-$tgt_lang.cln.$tgt_lang.sp  \
    --max-length 150 \
    --vocabs corp/vocab_new.yml corp/vocab_new.yml \
    ...
    --valid-sets wp.dev.ca.tags.sp wp.dev.ca.$tgt_lang.sp \
    --valid-translation-output data/valid.update{U}.output \
    --valid-mini-batch 16 --overwrite  \
    --beam-size 4 --normalize 0.6 \
    --log model/train_bt_balanced_from_scratch.new_vocab.log --valid-log model/valid.log \
