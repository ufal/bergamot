...
$marian/marian \
	--model model/model.ca-$tgt_lang.2.phon.npz  --type transformer \
    --train-sets ../../data/lang_pairs/ca-$tgt_lang/all_text.ca-oc.cln.ca.phon.phonsp ../../data/lang_pairs/ca-$tgt_lang/all_text.ca-oc.cln.oc.phonsp  \
    --max-length 150 \
    --vocabs corp/vocab.phon.yml corp/vocab.phon.yml \
    --mini-batch-fit -w 9300 --maxi-batch 1000 \
    --early-stopping 200 \
    --valid-freq 1000 --save-freq 1000 --disp-freq 100  --valid-script-path ./val.sh \
    --valid-metrics cross-entropy perplexity translation bleu-detok \
    --valid-sets wp.dev.ca.phonsp wp.dev.ca.oc.phonsp \
    --valid-translation-output data/valid.update{U}.output \
    --valid-mini-batch 16 --overwrite  \
    --beam-size 4 --normalize 0.6 \
    --log model/train_phon.log --valid-log model/valid.log \
